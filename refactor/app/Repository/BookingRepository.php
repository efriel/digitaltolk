...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    public function distanceFeed($data)
    {
        ...
        if ($admincomment || $session || $flagged || $manually_handled || $by_admin) {
            $affectedRows = Job::where('id', '=', $jobid)->update(array('admin_comments' => $admincomment, 'flagged' => $flagged, 'session_time' => $session, 'manually_handled' => $manually_handled, 'by_admin' => $by_admin));
            $response['status'] = 'success';
            $response['message'] = 'Record updated!';
            $response['rows'] = $affectedRows;
